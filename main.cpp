...
void core_only_map(const int &max_dim, const double &max_cdd)
{
    for (int dim = 10; dim <= max_dim; dim += 10)
    {
        ...
        myq.dump_statistic("statistic\\core_only_map.txt");
    }   
}
...
void core_grow_map(const int &max_dim, const double &cdd, const double &max_frac)
{
    for (int dim = 10; dim <= max_dim; dim += 10)
    {
        ...
        myq.dump_statistic("statistic\\core_grow_map.txt");
    }   
}
...
void core_only_curve(const double &max_cdd, const double &threshold)
{
    ...
    for (double cdd = max_cdd; cdd >= 0.049; cdd -= 0.05)
    {
        ...
        for (int dim = min_dim; dim <= 200; dim += (min_dim / 20 > 1) ? min_dim / 20 : 1)
        {
            ...
            {
                cout << "dim = " << dim << endl;
                myq.dump_statistic("statistic\\core_only_curve.txt");
                min_dim = dim;
                break;
            }
        }
...
void core_grow_curve(const double &cdd, const double &max_frac, const double &threshold)
{
    ...
    for (double frac = max_frac; frac >= 0.049; frac -= 0.05)
    {
        ...
        for (int dim = min_dim; dim <= 200; dim += (min_dim / 20 > 1) ? min_dim / 20 : 1)
        {
            ...
            {
                cout << "dim = " << dim << endl;
                myq.dump_statistic("statistic\\core_grow_curve.txt");
                min_dim = dim;
                break;
            }
        }
...
void save_aniso(const int &num, const int &dim, const double &cdd, const double &frac, const double &px=1, const double &py=1, const double &pz=1, const string &str="min")
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        cout << i << ": " << myq.volume_farction() << endl;
        myq.output(i, "structure\\" + to_string(dim) + "-" + to_string(frac) + "-" + to_string(cdd) + "-aniso");
    }
}
